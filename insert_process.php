...
if(isset($_REQUEST['confirmorder']) && $_GET['page'] == "checkoutconfirm")
 {
  ...
  $mtotal = $conn->real_escape_string((isset($_POST['mtotal']) ? $_POST['mtotal'] : ""));
	...
	$sid = session_id();
	$sql="SELECT SUM(quantity) FROM tblcart WHERE cart_session_id ='$sid'" ;
	$result = $conn->query($sql)or die($conn->error);
	while ($row = $result->fetch_assoc())
	{


		$cart_qtytl=number_format($row['SUM(quantity)']);

    $cart_qtyt=$row['SUM(quantity)'];
 		...
 		$stmt = $conn->stmt_init();
 		$stmt->prepare("INSERT INTO tblinvoice(customer_id,payment_id,alltotal_amount,totalqty,order_email) VALUES (?,?,?,?,?)");
    ...
    $aaa="SELECT order_id FROM tblinvoice WHERE customer_id = '$uid' ORDER BY order_id DESC ";
    ...
    $stmt = $conn->stmt_init();
    $stmt->prepare("INSERT INTO tblshipping(order_id,customer_id,shipping_name,shipping_address,phone) VALUES (?,?,?,?,?)");
...
$aa="SELECT order_id FROM tblinvoice WHERE customer_id= '$uid' ORDER BY order_id DESC LIMIT 1";
...
//$orderId = $order + 1;
	...
	if($uid!="")
	{
		...
		if ($orderId)
		{
			...
			for ($i = 0; $i < $numItem; $i++)
			{
				...
				$cartContent[$i]['total_price'];
		

				$sql1="INSERT INTO tbl_orderdetail (order_id,product_id,product_name,price,qty,amount)
				VALUES ('$orderId','{$cartContent[$i]['product_id']}','{$cartContent[$i]['product_name']}','{$cartContent[$i]['price']}','{$cartContent[$i]['quantity']}',
					'{$cartContent[$i]['total_price']}')";
			...
			$oqry = $conn->query("SELECT order_id FROM tblinvoice WHERE customer_id='$uid' ORDER BY order_id DESC LIMIT 1")or die($conn->error);
    ...
    $cust = "SELECT order_id, order_date, order_email FROM tblinvoice WHERE customer_id = '$uid' ORDER BY order_id DESC";
  ...
  // $dquery = $conn->query($del)or die($conn->error);
    ...
    $product_name = $abrow['product_name'];
      ...
      $no++;
			...
			$stmt = $conn->stmt_init();
			$stmt->prepare("SELECT o.alltotal_amount,s.shipping_address FROM tblinvoice AS o, tblshipping AS s WHERE s.order_id=o.order_id AND o.order_id= $orid ");
    ...
    /////////////////////////////////Mail to Customer end ////////////////////////////////
		...
		$del="DELETE FROM tblcart WHERE cart_session_id='$sid'";
		$dquery = $conn->query($del)or die($conn->error);
